...
final class Convolve(config: Convolve.Config) {
  ...
  def extractChannel2Mul(in: BufferedImage, arr: Array[Array[Double]], x: Int, y: Int, w: Int, h: Int, chan: Int, mul: Double): Unit = {
    val shift = chan * 8
    val iw = in.getWidth
    val ih = in.getHeight
    var y0 = 0
  ...
  def extractChannel2Add(in: BufferedImage, arr: Array[Array[Double]], x: Int, y: Int, w: Int, h: Int, chan: Int, mul: Double): Unit = {
    val shift = chan * 8
    val iw = in.getWidth
    val ih = in.getHeight
    var y0 = 0
  ...
  def renderFrame(fInA: File, fInB: File, fOut: File, agc: Double, prevGain: Gain): Gain = {
    ...
    val i4 = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB)
    val g4 = i4.createGraphics()
    g4.setColor(Color.black)
    g4.fillRect(0, 0, w, h)
    // g4.dispose()

//    g4.setColor(Color.white)
//    for (x <- kh until (w - kh) by kernel) {
//      for (y <- kh until (h - kh) by kernel) {
//        g4.fillRect(x, y, 1, 1) // kh, kh)
//      }
//    }
//    g4.dispose()

    // val g1 = i2.createGraphics()
    ...
    g4.setRenderingHint(RenderingHints.KEY_RENDERING    , RenderingHints.VALUE_RENDER_QUALITY)
    ...
    for (x <- 0 until w1 by wh) {
      for (y <- 0 until h1 by wh) {
        ...
        for (chan <- 0 until 3) {
          val b3 = bCh0(chan)
//          g4.setColor(Color.black)
//          g4.fillRect(x - kh, y - kh, kernel, kernel)
//          g4.setColor(Color.white)
//          g4.fillRect(x, y, 1, 1)
...
//          extractChannel2Mul(i1, arr = b1, x = x - kh, y = y - kh, w = kernel, h = kernel, chan = chan, mul = 0.9)
//          extractChannel2Add(i2, arr = b1, x = x - kh, y = y - kh, w = kernel, h = kernel, chan = chan, mul = 0.1)
...
//          g4.setColor(new Color(255, 255, 255, 128))
    ...
    val hasNoise = config.noise > 0

    for (x <- 0 until w) {
      for (y <- 0 until h) {
        val noise = if (hasNoise) math.random.linlin(0, 1, -config.noise, config.noise) else 0.0
        for (chan <- 0 until 3) {
          val row = bCh0(chan)(y)
