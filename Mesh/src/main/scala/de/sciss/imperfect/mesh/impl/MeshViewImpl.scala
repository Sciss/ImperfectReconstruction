...
import de.sciss.nuages.{BasicPanel, BasicSlider, ControlPanel, Nuages, NuagesContext, NuagesObj, NuagesPanel, OverlayPanel, ParamSpec, ScissProcs}
import de.sciss.osc
...
import scala.swing.{Action, BorderPanel, BoxPanel, Button, Component, GridBagPanel, Label, MenuItem, Orientation, TextField}
...
object MeshViewImpl {
  def apply[S <: Sys[S]](nuages: Nuages[S], nuagesConfig: Nuages.Config, scissConfig: ScissProcs.Config)
    ...
    val panel     = MeshPanel(nuages, nuagesConfig)
  ...
  private final class Impl[S <: Sys[S]](val panel: MeshPanel[S], transportView: View[S],
                                        sConfig: ScissProcs.Config)
                                       (implicit val cursor: stm.Cursor[S])
    extends MeshView[S] with ComponentHolder[Component] with AuralSystem.Client { impl =>
