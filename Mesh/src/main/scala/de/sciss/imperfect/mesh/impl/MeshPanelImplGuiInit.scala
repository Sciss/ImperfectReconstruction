...
import de.sciss.nuages.impl.{PanelLayout, RubberBandSelect}
import de.sciss.nuages.{ClickControl, ConnectControl, DragAndMouseDelegateControl, GlobalControl, NuagesShapeRenderer, PanControl, PrefuseAggregateLayout}
...
trait MeshPanelImplGuiInit[S <: Sys[S]] extends ComponentHolder[Component] {
  import MeshPanelImpl.{GROUP_NODES, GROUP_EDGES, AGGR_PROC, ACTION_COLOR, ACTION_LAYOUT, LAYOUT_TIME}
  import MeshPanel.{GROUP_GRAPH, GROUP_SELECTION, COL_NUAGES}

  // ---- abstract ----

  protected def keyControl: Control
  protected def main: MeshPanel[S]
...
//    val aggrRenderer = new PolygonRenderer(Constants.POLY_TYPE_CURVE)
    val aggrRenderer = new PolygonRenderer(Constants.POLY_TYPE_LINE)
